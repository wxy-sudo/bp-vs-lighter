...
class BackpackClient(BaseExchangeClient):
    ...
    async def place_open_order(self, contract_id: str, quantity: Decimal, direction: str) -> OrderResult:
        ...
        while retry_count < max_retries:
            ...
            if direction == 'buy':
                # For buy orders, place 10 ticks below best ask for better maker position
                order_price = best_ask - self.config.tick_size * 30
                side = 'Bid'
            else:
                # For sell orders, place 10 ticks above best bid for better maker position
                order_price = best_bid + self.config.tick_size * 30
                side = 'Ask'
