...
class HedgeBot:
    """Backpack maker + Lighter hedge bot with basic spread triggers."""
    ...
    async def trading_loop(self):
        ...
        while not self.stop_flag:
            ...
            if long_signal and self.backpack_position < self.max_position:
                self.logger.info(f"📈 Long signal triggered | spread={long_spread}")
                await self._execute_long_trade()
            elif short_signal and self.backpack_position > -self.max_position:
                self.logger.info(f"📉 Short signal triggered | spread={short_spread}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Backpack ↔ Lighter spread arbitrage bot")
    parser.add_argument("--ticker", type=str, default="BTC", help="Symbol to trade (default: BTC)")
    parser.add_argument("--size", type=str, required=True, help="Order size per trade (e.g. 0.002)")
    parser.add_argument("--max-position", type=str, default=None, help="Maximum absolute Backpack position")
    parser.add_argument("--long-threshold", type=str, default="5", help="Trigger when Lighter bid - Backpack bid exceeds this value")
    parser.add_argument("--short-threshold", type=str, default="5", help="Trigger when Backpack ask - Lighter ask exceeds this value")
    parser.add_argument("--use-dynamic-thresholds", action="store_true", help="Use rolling z-score thresholds instead of fixed spreads")
    parser.add_argument("--spread-window", type=int, default=200, help="Rolling window size for dynamic thresholds")
    parser.add_argument("--spread-entry-z", type=float, default=2.0, help="Z-score to trigger entries when using dynamic thresholds")
    parser.add_argument("--spread-exit-z", type=float, default=0.5, help="Z-score to re-arm signals (hysteresis) in dynamic mode")
